//decalramos las cariables para los controles
...
function Mostrar(){
    //declarando unca variable para contar el numero de filas
    var i = 0;
    //selleciono el tbody de la tabla donde voy a mostrar la imformacion
    ...
    db.once("value", function(snapshot) {
        if(snapshot.exists()) {
          snapshot.forEach(function (data) {
            ...
            //declarando cariable para los titulos
...
//llamamos a la funcion mostrar cuan carga la pagina
//window.onload = Mostrar;
//llamammos al procedimiento parar cargar combo
...
function Registrar(){
    ...
    function Buscar(codigo){
        ...
        db.once("value", function(snapshot) {
            snapshot.forEach(function (data) {
                //declaramos una variable para obtener el ID(KEY) de la tanla
                var key = data.key;
                //veridicando si el codifo es igual al ID de la tabla
                if (key == codigo){
                    var cod = key;
                    var nom = data.val().nombre;
                    var ape = data.val().apellido;
    ...
    function Actualizar(){
        ...
        //le asignamos los calores que se van a actualizar
        db.update({
            nombre: nom,
            apellido: ape,
            dni: dni,
    ...
    //creamos un proceduimiento para crear
    function Eliminar(codigo){
        //preguntamos si se quiere eliminar el registro
        var result = confirm("Estas seguro que quyieres eliminar el registro????");
        //Evaluanmos la respuesta
    if (result) {
...
//llamamos al procedimieto para registrar
btnRegistrar.addEventListener("click",Registrar);
btnActualizar.addEventListener("click",Actualizar);
//llamando a la cuncion para registrar del firevase
//writeUserData()